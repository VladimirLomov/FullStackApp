...
class  FileController {
    ...
    async createDir(req,res){
        try {
            const {name,type,parent} = req.body
             ...
             const parentFile = await File.findOne({_id: parent}) // найдем родительский файл
    ...
    async uploadFile(req,res) {
        try{
            ...
            if(user.usedSpase + file.size > user.diskSpace) {
                return res.status(400).json({message:"no space on the disk"})
            }

            user.usedSpase = user.usedSpace + file.size
